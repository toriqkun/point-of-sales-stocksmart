...
export function LandingContent({ initialStats }: LandingContentProps) {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 selection:bg-indigo-100 selection:text-indigo-900 overflow-x-hidden">
      ...
      <nav className={cn(
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
          <div className="flex items-center space-x-2 sm:space-x-3 group cursor-pointer">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-indigo-600 rounded-xl sm:rounded-2xl flex items-center justify-center text-white font-black shadow-xl shadow-indigo-200 group-hover:rotate-6 transition-transform">S</div>
            <span className="text-xl sm:text-2xl font-black text-slate-900 tracking-tighter">StockSmart<span className="text-indigo-600">.</span></span>
          </div>
          <div className="flex items-center">
            <Link 
              href="/login" 
              className="bg-indigo-500 text-white flex items-center space-x-2 px-4 py-2 sm:px-6 sm:py-3 rounded-xl sm:rounded-2xl shadow-xl shadow-slate-200 hover:bg-indigo-600 hover:shadow-indigo-100 transition-all font-bold active:scale-95 text-sm sm:text-base"
            >
              <span className="hidden min-[385px]:inline">Login Dashboard</span>
              <span className="min-[385px]:hidden">Login</span>
      ...
      {/* Hero Section */}
      <section className="relative pt-25 sm:pt-32 lg:pt-35 pb-20 px-6">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
          <div className="flex-1 space-y-8 text-center lg:text-left animate-in fade-in slide-in-from-bottom-10 duration-700">
            <div className="inline-flex items-center space-x-2 bg-indigo-50 text-indigo-600 px-5 py-2 rounded-full border border-indigo-100 shadow-sm animate-bounce">
              ...
              <span className="text-[10px] sm:text-xs font-black uppercase tracking-widest">The Smartest POS Ecosystem</span>
            </div>
            
            <h1 className="text-5xl sm:text-6xl lg:text-[84px] font-black text-slate-900 leading-[0.95] tracking-tighter">
              Solusi Cerdas <br />
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-6">
              <Link href="/register" className="bg-indigo-600 text-white text-lg sm:text-xl px-8 sm:px-10 py-4 sm:py-4.5 rounded-3xl w-full sm:w-auto shadow-2xl shadow-indigo-200 hover:scale-[1.02] active:scale-95 transition-all font-black flex items-center justify-center space-x-3">
              ...
              <div className="flex flex-col items-center lg:items-start space-y-2">
                <div className="flex -space-x-3 items-center">
                  {initialStats.recentAvatars.length > 0 ? (
                    initialStats.recentAvatars.map((url, i) => (
                    ...
                    // Default fallback images that look real
                    [1,2,3,4].map(i => (
                      <div key={i} className="w-10 h-10 rounded-full border-2 border-white bg-slate-200 overflow-hidden relative shadow-sm">
                  ...
                  {initialStats.ownerCount > 0 && (
                    <div className="w-10 h-10 rounded-full border-2 border-white bg-indigo-600 flex items-center justify-center text-[10px] text-white font-black shadow-sm">
                      +{initialStats.ownerCount}
                    </div>
                  )}
                </div>
                <p className="text-sm font-bold text-slate-500">
                  Bergabung dengan <span className="text-slate-900">{initialStats.ownerCount > 0 ? initialStats.ownerCount : "Ratusan"}</span> pengusaha lainnya
                </p>
              </div>
            </div>
          </div>
          
          <div className="flex-1 relative animate-in fade-in zoom-in duration-1000 delay-200 w-full lg:w-auto">
            ...
            <div className="relative group">
              ...
              <div className="relative card p-2 sm:p-3 rotate-1 hover:rotate-0 transition-transform duration-500 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] border-white/50 bg-white/40 backdrop-blur-md">
                <div className="overflow-hidden rounded-2xl sm:rounded-3xl bg-slate-900 border border-white/20 relative aspect-video">
                  <Image 
                    src="/bg1.jpg" 
      ...
      <section id="features" className="py-20 sm:py-32 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center max-w-3xl mx-auto mb-16 sm:20 space-y-4 sm:space-y-6">
            <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 tracking-tight">Kekuatan Penuh untuk <br /><span className="text-indigo-600">Skalabilitas Bisnis</span></h2>
            <p className="text-lg sm:text-xl text-slate-500 font-medium leading-relaxed">Dirancang dengan teknologi terbaru untuk memberikan pengalaman manajemen paling halus dan akurat.</p>
          </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="bg-slate-900 rounded-[2.5rem] sm:rounded-[3rem] p-10 lg:p-20 relative overflow-hidden shadow-2xl shadow-indigo-200">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12 text-center lg:text-left">
              <div className="space-y-6 max-w-2xl">
                <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black text-white leading-tight">Siap Untuk Membuat Bisnis Anda <span className="text-indigo-400">Lebih Pintar?</span></h2>
                <p className="text-lg sm:text-xl text-slate-400 font-medium">Daftar sekarang dan nikmati kecanggihan manajemen stok masa depan.</p>
              ...
              <Link href="/register" className="bg-white text-slate-900 hover:bg-slate-100 px-10 sm:px-12 py-5 sm:py-6 rounded-2xl sm:rounded-3xl text-xl sm:text-2xl font-black shadow-2xl transition-all active:scale-95 shrink-0 w-full sm:w-auto">
                Gunakan Gratis
              </Link>
            </div>
          </div>
...
function FeatureCard({ icon, title, desc, gradient, newBadge }: any) {
  return (
    <div className="group relative p-8 sm:p-10 rounded-4xl sm:rounded-[2.5rem] bg-white border border-slate-100 hover:border-indigo-100 hover:shadow-2xl hover:shadow-slate-200 transition-all duration-500 hover:-translate-y-2 overflow-hidden">
      ...
      <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-slate-50 group-hover:bg-white group-hover:shadow-lg flex items-center justify-center mb-6 sm:mb-8 group-hover:scale-110 transition-all duration-500">
        {icon}
      </div>
      <div className="flex items-center space-x-3 mb-4">
        <h3 className="text-xl sm:text-2xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors">{title}</h3>
      ...
      <p className="text-sm sm:text-base text-slate-500 font-medium leading-relaxed group-hover:text-slate-600 transition-colors">{desc}</p>
    </div>
  );
}
